import type { authorizeWritingRequest } from "../_shared/writingAiEdge.ts";
...
export function createCompetitiveIntelligenceHandler(
  dependencies = {
    createClient,
    generateGeminiContent,
    extractTextFromGeminiResponse,
  ...
  const { createClient, generateGeminiContent, extractTextFromGeminiResponse } =
    dependencies;
  return async (
    req: Request,
    ...
    try {
      ...
      // Scope data access to the same verified organization as the writing key.
      // Background callers must send an organization ID; global runs are rejected.
      ...
      console.log(
        ...
        } organizations with competitive intelligence enabled`,
      );
      ...
      for (const org of enabledOrgs || []) {
        ...
        if (competitorsError) {
          console.error(
            `Error fetching competitors for ${org.organization_id}:`,
            competitorsError,
          );
          continue;
        }
        ...
        if (orgError) {
          console.error(
            `Error fetching org data for ${org.organization_id}:`,
            orgError,
          );
          continue;
        }
        ...
        const brandVoice = brandKnowledge?.find((k) =>
          k.knowledge_type === "brand_voice"
        )?.content || "";
        const vocabulary = brandKnowledge?.find((k) =>
          k.knowledge_type === "vocabulary"
        )?.content || "";
        ...
        for (const competitor of competitors || []) {
          ...
          try {
            ...
            if (!scrapeResponse.ok) {
              console.error(
                `Failed to fetch ${competitor.competitor_url}: ${scrapeResponse.status}`,
              );
              continue;
            }
...
Be specific, actionable, and brand-aware.`;
            ...
            console.log(
              `✅ Analyzed ${competitor.competitor_name}: ${insights.length} insights`,
            );
          } catch (error) {
            console.error(
              `Error analyzing ${competitor.competitor_name}:`,
              error,
            );
          }
        }
