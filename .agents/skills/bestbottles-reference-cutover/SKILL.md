...
description: >-
  RETIRED PROCEDURE — do NOT run the transparent cutover. Transparent /
  background-removed Best Bottles references were retired on 2026-06-22;
  `generate-madison-image` hard-rejects them (RETIRED_TRANSPARENT tokens include
  `clean-references/` and `transparent`). The live contract is ONE flattened
  Photoshop export with the original/source background
  (`{graceSku}__{websiteSku}__pdp-main__v001.png`). For reference cutover /
  repointing / "wrong reference" requests, use
  `scripts/best-bottles/remediate-cylinder-flattened-references.ts` as the model:
  upload flattened exports from the Best-Bottles repo
  `pipeline/aios-shopify-pdp-images/00-input/reference-flattened/<product-group-slug>/`
  to `generated-images/<org>/best-bottles/reference-images/` and repoint
  `best_reference_candidate_path` there. This file is kept for the still-valid
  architecture notes (which column the pipeline reads, URL validator, edge fetch).
---

# ⚠️ RETIRED: Best Bottles clean-reference cutover (per family)

> **DO NOT RUN THE PROCEDURE BELOW.** On **2026-06-22** transparent /
> background-removed references were deliberately retired
> (`public/data/bb-cylinder-transparent-supabase-cleanup-result.json`): the 333
> Cylinder rows were nulled, the `clean-references/cylinder/` storage was
> deleted, and `generate-madison-image` +
> `src/lib/bestBottlesReferenceFilters.ts` now **hard-reject** any reference
> whose URL/metadata matches retired tokens (`clean-references/`,
> `transparent`, `background-removed`, `paper-doll`, `mask-*`, …).
>
> **Running this cutover on 2026-07-04 re-introduced the retired references and
> blocked all Cylinder generation** until it was remediated the same day
> (`scripts/best-bottles/remediate-cylinder-flattened-references.ts`, report in
> `tmp/bestbottles-generation/cylinder-flattened-remediation-report.json`).
>
> **The live contract instead:** each SKU job's
> `best_reference_candidate_path` must point at ONE **flattened product-truth
> export with the original/source background**, named
> `{graceSku}__{websiteSku}__pdp-main__v001.png`, sourced from the Best-Bottles
> repo at `pipeline/aios-shopify-pdp-images/00-input/reference-flattened/
> <product-group-slug>/`, uploaded to the public
> `generated-images/<org>/best-bottles/reference-images/` prefix. SKUs with no
> flattened export stay NULL (`needs_reference`) and wait on the Cowork
> flattened-export lane — never backfill them from transparent or legacy
> renders.
>
> The sections below are retained ONLY for the still-true architecture facts
> (the job-table column is what the UI + edge function read; the URL must be a
> public https PNG; the validator/edge constraints). Treat every instruction to
> upload or repoint TRANSPARENT references as historical.

# [HISTORICAL] Original skill text
