...
- Four physical-review boundaries are recorded: split direct-bulb and tassel atomizers, quarantine the clear-protected lotion pump for responsibility decomposition, keep 20-400 dropper pipette length body-contextual, and require a fluted/flanged profile for 8-425 short caps instead of reusing the smooth renderer.
- Seven distinct dimension-calibrated cap candidate families now cover 23 catalog appearances. The two short 8-425 caps use a dedicated fluted/flanged mesh instead of the smooth renderer; its flute count remains an explicit source-derived review parameter rather than verified tooling truth. Every variant within its own family has byte-identical clamped alpha, but all seven profiles remain review-only: geometry lock, production eligibility, family placement, Current Release, and Sanity state are all unchanged.
