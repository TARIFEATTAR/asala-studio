...
- The 15-415 kit now also has two dimension-calibrated authority-candidate families. Five exposed-head appearances share one source-aspect mask at the verified 20 mm width, and five protective-overcap appearances share one 20 × 41 mm Blender mask. Every candidate is exact-alpha clamped and has been rendered against the five locked CYL-9ML bodies as visual scale context only. The context bodies are 17-415, so those sheets make no compatibility or family-fit claim for the 15-415 components. The dip tube remains body-contextual. Real-file topology cleanup is source-specific and records one 77,022-pixel retained head plus nine 4–13 pixel detached artifacts; no global island threshold was introduced. Named geometry review, compatible-family fit, Current Release, and Sanity remain unchanged.
- The compound-component gate now covers 20 catalog review lanes and 83 source identities: two pump lanes, six sprayer lanes, six dropper lanes, four bulb-sprayer lanes, one combined bulb-sprayer/sprayer lane, and one compound applicator lane. Fifteen lanes are source-ready, three are source-incomplete, and four already have explicit physical-review decisions. This queue does not change the 309 source-backed shot-list baseline; the final reusable-plate delta remains intentionally unresolved until each responsibility map is reviewed.
...
- Those twelve families now have one deterministic review book grouped into six neck-finish pages plus a mobile-friendly overview. Its manifest SHA-binds all recipes, candidate manifests, source review sheets, and output pages while explicitly preserving review-only state. See `PARAMETRIC-COMPONENT-REVIEW-BOOK.md`.
