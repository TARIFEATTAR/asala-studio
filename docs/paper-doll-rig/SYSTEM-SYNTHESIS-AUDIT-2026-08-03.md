...
- Eight distinct dimension-calibrated cap candidate families now cover 29 catalog appearances. The two short 8-425 caps use a dedicated fluted/flanged mesh instead of the smooth renderer; its flute count remains an explicit source-derived review parameter rather than verified tooling truth. The six 20-400 tall roll-on cap appearances share verified 23 × 35 mm dimensions and one smooth review profile. Every variant within its own family has byte-identical clamped alpha, but all eight profiles remain review-only: geometry lock, production eligibility, family placement, Current Release, and Sanity state are all unchanged.
