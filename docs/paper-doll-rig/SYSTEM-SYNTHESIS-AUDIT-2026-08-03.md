...
- Four distinct dimension-calibrated cap candidate families now cover 16 catalog appearances: nine 13-415 roll-on overcaps, two 13-415 standard caps, two 15-415 standard caps, and three 18-415 short caps. The reusable Blender camera now frames from both physical width and height so squat parts cannot be silently cropped. Every variant within its own family has byte-identical clamped alpha, but all four profiles remain review-only: geometry lock, production eligibility, family placement, Current Release, and Sanity state are all unchanged.
