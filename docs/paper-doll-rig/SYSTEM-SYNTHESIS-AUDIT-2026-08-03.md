...
- Two distinct dimension-calibrated 13-415 cap candidate families now cover 11 catalog appearances: nine roll-on overcaps and two standard caps. Each family uses its own 17 × 24 mm parametric Blender profile; matching nominal dimensions do not imply shared geometry. Every variant within its family has byte-identical clamped alpha, but both profiles remain review-only: geometry lock, production eligibility, family placement, Current Release, and Sanity state are all unchanged.
