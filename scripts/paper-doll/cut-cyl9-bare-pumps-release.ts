...
const RELEASE_VERSION = "1.3.4-bare-pumps.2";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-BARE-PUMPS-RELEASE-2";
const PUMP_LOCK_PATH = "docs/paper-doll-rig/cyl9-bare-pumps-v2-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-bare-pumps-v2-remote-release.json";
const EXPECTED_SOURCE_RELEASE = "1.3.3-bare-pumps.1";
...
async function loadPumpAssets(client: SupabaseClient, organizationId: string, pumpLock: JsonRecord) {
  const expected = pumpLock.pumpComponentVersions as JsonRecord[];
  if (expected.length !== 3) throw new Error("The pump lock must record exactly three bare versions.");
  const versions = await client.from("paper_doll_component_versions")
    .select("*")
    .eq("organization_id", organizationId)
    .in("id", expected.map(({ componentVersionId }) => componentVersionId));
  if (versions.error || versions.data.length !== 3) {
    throw versions.error ?? new Error("The three bare pump versions are not available remotely.");
  }
  const components = await client.from("paper_doll_components")
    .select("*")
    .eq("organization_id", organizationId)
    .in("id", [...new Set(versions.data.map(({ component_id }) => component_id))]);
  if (components.error || components.data.length !== 3) {
    throw components.error ?? new Error("The three pump component identities are incomplete.");
  }
  return expected.map((locked) => {
    ...
    if (
      ...
      throw new Error(`Pump ${locked.variantKey} differs from its immutable lock evidence.`);
    }
    return {
...
async function main(): Promise<void> {
  ...
  const carriedAssets = currentManifest.assets.filter((asset: JsonRecord) => asset.slot !== "pump");
  if (carriedAssets.length !== 23) throw new Error("Current Release must carry exactly twenty-three non-pump layers forward.");
  ...
  const manifest = {
    ...
    qaEvidence: [
      ...
      ...pumpAssets.map((asset) => ({
        evidenceId: `bare-pump-v2-${asset.imageSha256.slice(0, 16)}`,
        subjectId: asset.componentVersionId,
        gateKey: "bare-pump-v2",
        gateVersion: "exact-alpha-identity",
        status: "passed",
        blocking: true,
  ...
  const evidence = {
    ...
    releaseVersion: RELEASE_VERSION,
    sourceGitCommit,
    pumpComponentVersionIds: pumpAssets.map(({ componentVersionId, variantKey, placementVersionId }) => ({ componentVersionId, variantKey, placementVersionId })),
    releaseCut: cut.data,
    sanity: {
      draftQueued: true,
      draftSynced: false,
