...
/**
 * Cut the CYL-9ML bare-pumps release: replace the three capped-dispenser
 * lotion pump layers of the Current Release (1.3.2-roller-seat.1) with the
 * bare-pump v1 versions recorded in
 * docs/paper-doll-rig/cyl9-bare-pumps-v1-lock.json. Every other layer is
 * carried forward unchanged. Each pump carries its own per-colour identity
 * placement lock.
 */
...
const RELEASE_VERSION = "1.3.8-roller-light.1";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-ROLLER-LIGHT-RELEASE";
const PUMP_LOCK_PATH = "docs/paper-doll-rig/cyl9-roller-light-v1-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-roller-light-v1-remote-release.json";
const EXPECTED_SOURCE_RELEASE = "1.3.7-cap-crown.1";
...
async function loadPumpAssets(client: SupabaseClient, organizationId: string, pumpLock: JsonRecord) {
  const expected = pumpLock.rollerComponentVersions as JsonRecord[];
  if (expected.length !== 2) throw new Error("The roller lock must record exactly two harmonized versions.");
  ...
  if (versions.error || versions.data.length !== 2) {
    throw versions.error ?? new Error("The two harmonized roller versions are not available remotely.");
  }
  const components = await client.from("paper_doll_components")
    .select("*")
    .eq("organization_id", organizationId)
    .in("id", [...new Set(versions.data.map(({ component_id }) => component_id))]);
  if (components.error || components.data.length !== 1) {
    throw components.error ?? new Error("The roller component identity is incomplete.");
  }
...
async function main(): Promise<void> {
  ...
  const pumpLock = JSON.parse(await readFile(PUMP_LOCK_PATH, "utf8"));
  const { contentSha256, ...lockPayload } = pumpLock;
  if (sha256(JSON.stringify(lockPayload)) !== contentSha256) {
    throw new Error("The immutable bare-pump evidence hash no longer matches.");
  }
  const [current, pumpAssets] = await Promise.all([
    loadCurrentRelease(client, organizationId),
    loadPumpAssets(client, organizationId, pumpLock),
  ]);
  ...
  const replacedIds = new Set(pumpAssets.map((asset: JsonRecord) => `${asset.slot}:${asset.variantKey}`));
  const carriedAssets = currentManifest.assets.filter((asset: JsonRecord) => !replacedIds.has(`${asset.slot}:${asset.variantKey}`));
  if (carriedAssets.length !== 24) throw new Error("Current Release must carry exactly twenty-four non-roller layers forward.");
  const mappings = buildReleaseAssemblyMappings(loadCyl9ComponentFactory());
  const assets = [...carriedAssets, ...pumpAssets]
  ...
  const manifest = {
    ...
    qaEvidence: [
      ...
      ...pumpAssets.map((asset) => ({
        evidenceId: `roller-light-v1-${asset.imageSha256.slice(0, 16)}`,
        subjectId: asset.componentVersionId,
        gateKey: "roller-light-v1",
        gateVersion: "exact-alpha-neutral-anchor",
        status: "passed",
        blocking: true,
  ...
  const plan = {
    manifest,
    readiness,
    selectedComponents: pumpAssets.map((asset) => ({
      slot: "roller",
  ...
  if (!execute) {
    process.stdout.write(`${JSON.stringify({
      ...
      pumpBounds: pumpAssets.map(({ variantKey, alphaBounds }) => ({ variantKey, alphaBounds })),
      catalogReadiness: {
  ...
  const evidence = {
    ...
    releaseVersion: RELEASE_VERSION,
    sourceGitCommit,
    rollerComponentVersionIds: pumpAssets.map(({ componentVersionId, variantKey, placementVersionId }) => ({ componentVersionId, variantKey, placementVersionId })),
    releaseCut: cut.data,
    sanity: {
      draftQueued: true,
      draftSynced: false,
