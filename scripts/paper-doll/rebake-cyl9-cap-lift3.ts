...
const CONFIRMATION = "CYL9-CAP-LIFT-V3";
...
const OUT_ROOT = "outputs/paper-doll-cap-lift-v3";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-cap-lift-v3-lock.json";
const LIFT_PX = 4;
...
async function main() {
  ...
  if (manifest.releaseVersion !== "1.3.9-cap-lift.2") {
    throw new Error("Cap-lift sources must come from the 1.3.9 export.");
  }
  ...
  for (const variantKey of CAP_VARIANTS) {
    ...
    const layerPath = resolve(OUT_ROOT, `cap__17-415__${variantKey}__lift-v3.png`);
    ...
    items.push({
      ...
      version: {
        versionKey: `cap-lift-v3-${layerSha.slice(0, 12)}`,
        materialVariant: asset.materialVariant,
        storageBucket: "paper-doll-approved",
        imagePath: `${organizationId}/${FAMILY_KEY}/cap-lift-v3/${variantKey}/${layerSha}.png`,
        imageSha256: layerSha,
        geometryMaskPath: `${organizationId}/${FAMILY_KEY}/cap-lift-v3/authority/${newMaskSha}.png`,
      ...
      qaResults: [
        ...
        },
        {
          ...
          gateVersion: "cap-lift-v3-identity",
          qaStatus: "passed",
          blocking: true,
          calibratedWith: [newMaskSha],
