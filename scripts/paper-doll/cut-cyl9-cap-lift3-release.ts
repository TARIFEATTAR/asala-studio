...
const RELEASE_VERSION = "1.3.10-cap-lift.3";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-CAP-LIFT-RELEASE-3";
const CAP_LOCK_PATH = "docs/paper-doll-rig/cyl9-cap-lift-v3-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-cap-lift-v3-remote-release.json";
...
export function buildCapLiftReleasePlan(input: {
  ...
  const qaEvidence = [
    ...
    ...input.capAssets
      .filter((asset) => asset.versionOrigin === "cap-lift-v3")
      .map((asset) => ({
        evidenceId: `cap-lift-v3-${asset.imageSha256.slice(0, 16)}`,
...
async function loadCapAssets(
  ...
  return capLock.capComponentVersions.map((locked: JsonRecord) => {
    ...
    return {
      ...
      versionOrigin: String(version.version_key).startsWith("cap-lift-v3")
        ? "cap-lift-v3"
        : "unexpected",
    };
  });
...
async function main(): Promise<void> {
  ...
  const newCapCount = capAssets.filter(({ versionOrigin }) => versionOrigin === "cap-lift-v3").length;
  if (newCapCount !== 10) {
    throw new Error(`Expected ten cap-lift v3 caps in the lock; found ${newCapCount}.`);
  }
