...
const RELEASE_VERSION = "1.3.1-clean-caps.1";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-CLEAN-CAPS-RELEASE";
const CAP_LOCK_PATH = "docs/paper-doll-rig/cyl9-adobe-cap-v2-remote-placement-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-clean-caps-v2-remote-release.json";
...
export function buildCleanCapsReleasePlan(input: {
  ...
  if (
    ...
    throw new Error("The CYL-9ML clean-caps inputs are incomplete or stale.");
  }
  ...
  const qaEvidence = [
    ...
    ...input.capAssets
      .filter((asset) => asset.versionOrigin === "adobe-clean-v2")
      .map((asset) => ({
        evidenceId: `adobe-cap-v2-${asset.imageSha256.slice(0, 16)}`,
        subjectId: asset.componentVersionId,
        gateKey: "adobe-cap-v2",
...
async function loadCapAssets(
  ...
  return capLock.capComponentVersions.map((locked: JsonRecord) => {
    ...
    return {
      ...
      versionOrigin: String(version.version_key).startsWith("adobe-clean-v2")
        ? "adobe-clean-v2"
        : "source-backed-v1",
    };
  });
...
async function main(): Promise<void> {
  ...
  const newCapCount = capAssets.filter(({ versionOrigin }) => versionOrigin === "adobe-clean-v2").length;
  if (newCapCount !== 9) {
    throw new Error(`Expected nine Adobe-clean v2 caps in the lock; found ${newCapCount}.`);
  }
  const sourceGitCommit = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
  const plan = buildCleanCapsReleasePlan({
