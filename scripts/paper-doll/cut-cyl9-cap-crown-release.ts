...
const RELEASE_VERSION = "1.3.7-cap-crown.1";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-CAP-CROWN-RELEASE";
const CAP_LOCK_PATH = "docs/paper-doll-rig/cyl9-cap-crown-v1-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-cap-crown-v1-remote-release.json";
...
export function buildCapCrownReleasePlan(input: {
  ...
  if (
    ...
    throw new Error("The CYL-9ML cap-crown inputs are incomplete or stale.");
  }
  ...
  const qaEvidence = [
    ...
    ...input.capAssets
      .filter((asset) => asset.versionOrigin === "cap-crown-v1")
      .map((asset) => ({
        evidenceId: `cap-crown-v1-${asset.imageSha256.slice(0, 16)}`,
        subjectId: asset.componentVersionId,
        gateKey: "cap-crown-v1",
        gateVersion: "exact-alpha-height-exact",
        status: "passed",
        blocking: true,
...
async function loadCapAssets(
  ...
  return capLock.capComponentVersions.map((locked: JsonRecord) => {
    ...
    return {
      ...
      versionOrigin: String(version.version_key).startsWith("cap-crown-v1")
        ? "cap-crown-v1"
        : String(version.version_key).startsWith("cap-lift-v1")
          ? "cap-lift-v1"
          : "unexpected",
    };
  });
...
async function main(): Promise<void> {
  ...
  const newCapCount = capAssets.filter(({ versionOrigin }) => versionOrigin === "cap-crown-v1").length;
  const retainedCount = capAssets.filter(({ versionOrigin }) => versionOrigin === "cap-lift-v1").length;
  if (newCapCount !== 9 || retainedCount !== 1) {
    throw new Error(`Expected nine crown refits plus retained WHT; found ${newCapCount}+${retainedCount}.`);
  }
  const sourceGitCommit = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
  const plan = buildCapCrownReleasePlan({
