...
const CONFIRMATION = "CYL9-CAP-LIFT-V1";
const FAMILY_KEY = "CYL-9ML";
const GEOMETRY_KEY = "closure__17-415__rollon-overcap__v2";
const EXPORT_ROOT = "outputs/paper-doll-release-export/1.3.5-light-harmonized.1";
const OUT_ROOT = "outputs/paper-doll-cap-lift-v1";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-cap-lift-v1-lock.json";
const LIFT_PX = 2;
...
async function main() {
  ...
  if (manifest.releaseVersion !== "1.3.5-light-harmonized.1") {
    throw new Error("Cap-lift sources must come from the 1.3.5 export.");
  }
  ...
  for (const variantKey of CAP_VARIANTS) {
    ...
    const layerPath = resolve(OUT_ROOT, `cap__17-415__${variantKey}__lift-v1.png`);
    ...
    items.push({
      ...
      version: {
        versionKey: `cap-lift-v1-${layerSha.slice(0, 12)}`,
        materialVariant: asset.materialVariant,
        storageBucket: "paper-doll-approved",
        imagePath: `${organizationId}/${FAMILY_KEY}/cap-lift-v1/${variantKey}/${layerSha}.png`,
        imageSha256: layerSha,
        geometryMaskPath: `${organizationId}/${FAMILY_KEY}/cap-lift-v1/authority/${newMaskSha}.png`,
      ...
      qaResults: [
        ...
        },
        {
          ...
          gateVersion: "cap-lift-v1-identity",
          qaStatus: "passed",
          blocking: true,
          calibratedWith: [newMaskSha],
