...
export { buildReleaseAssemblyMappings } from "./register-cyl9-capped-dispenser-release";
...
const RELEASE_VERSION = "1.3.0-complete-family.1";
const RENDERER_VERSION = "paper-doll-complete-family-v1";
const SANITY_PUBLIC_DOCUMENT_ID = "d5291f24-f02b-4fb7-aa99-78c5f63d8c9d";
const CONFIRMATION = "CYL9-COMPLETE-FAMILY-RELEASE";
const CAP_LOCK_PATH = "docs/paper-doll-rig/cyl9-source-cap-v1-remote-placement-lock.json";
const EVIDENCE_PATH = "docs/paper-doll-rig/cyl9-complete-family-v1-remote-release.json";
...
export function buildCompleteCyl9ReleasePlan(input: {
  ...
  placementVersionIdsByGeometryFamily: Record<string, string>;
}) {
  if (
    ...
    || input.currentManifest.assets?.length !== 16
    ...
    throw new Error("The CYL-9ML completion inputs are incomplete or stale.");
  }
  ...
  const currentAssets = input.currentManifest.assets.map((asset: JsonRecord) => {
    if (asset.slot === "body" || asset.placementVersionId) return asset;
    const placementVersionId = input.placementVersionIdsByGeometryFamily[asset.geometryFamilyId];
    return placementVersionId ? { ...asset, placementVersionId } : asset;
  });
  const assets = [...currentAssets, ...input.capAssets]
  ...
  const qaEvidence = [
    ...
    ...input.capAssets.map((asset) => ({
      evidenceId: `source-cap-v1-${asset.imageSha256.slice(0, 16)}`,
      subjectId: asset.componentVersionId,
      gateKey: "source-cap-v1",
      gateVersion: "exact-alpha-five-body-y-neg-3",
      status: "passed",
      blocking: true,
...
async function loadCapAssets(
  ...
  if (
    ...
    || Number(placement.data.translate_y_px) !== -3
...
async function loadPlacementTruth(client: SupabaseClient, organizationId: string) {
  const placements = await client.from("paper_doll_placement_versions")
    .select("id,fitment_geometry_key,authority_mask_sha256")
    .eq("organization_id", organizationId)
    .eq("family_key", FAMILY_KEY);
  if (placements.error) throw placements.error;
  const approvals = await client.from("paper_doll_placement_approvals")
    .select("placement_version_id")
    .eq("organization_id", organizationId)
    .in("placement_version_id", placements.data.map(({ id }) => id));
  if (approvals.error) throw approvals.error;
  const approved = new Set(approvals.data.map(({ placement_version_id }) => placement_version_id));
  const result: Record<string, string> = {};
  for (const placement of placements.data) {
    if (!approved.has(placement.id)) continue;
    const existing = result[placement.fitment_geometry_key];
    if (existing && existing !== placement.id) {
      throw new Error(`Multiple approved placement versions exist for ${placement.fitment_geometry_key}; explicit reconciliation is required.`);
    }
    result[placement.fitment_geometry_key] = placement.id;
  }
  return result;
}
...
async function main(): Promise<void> {
  ...
  const [current, capAssets, placementVersionIdsByGeometryFamily] = await Promise.all([
    loadCurrentRelease(client, organizationId),
    loadCapAssets(client, organizationId, capLock),
    loadPlacementTruth(client, organizationId),
  ]);
  ...
  const plan = buildCompleteCyl9ReleasePlan({
  ...
  const evidence = {
    ...
    sourceGitCommit,
    capPlacementVersionId: capLock.placementVersionId,
    capComponentVersionIds: capAssets.map(({ componentVersionId, variantKey }) => ({ componentVersionId, variantKey })),
